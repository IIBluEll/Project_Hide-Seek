...
- `PATROL` 진입 후 새로운 시각·청각 증거나 Director Hint 없이 5초가 지나면 초당 2씩 자연 감소한다.
- `PATROL`을 벗어나면 감소를 중단하고, 다음 `PATROL` 진입 시 대기 시간을 처음부터 계산한다.
...
`PATROL` 진입 시 Chase AI의 현재 위치가 포함된 Zone을 판정하고, 해당 Zone의 `COVERAGE` 지점을 순찰 경로로 사용한다. 첫 지점은 현재 위치에서 가장 가까운 지점으로 선택한다. 현재 위치가 어떤 Zone에도 포함되지 않거나 해당 Zone에 활성화된 `COVERAGE` 지점이 없으면 기존 전역 순찰 지점을 fallback으로 사용한다. 이 순찰 경로 선정에는 플레이어 위치와 플레이어 Zone을 사용하지 않는다.
...
추격 중 시야가 끊기면 설정된 `VisualLoseTime` 동안 마지막으로 설정된 추격 목적지를 향해 계속 이동한다. 유예 시간 안에 시야를 다시 확보하면 추격을 유지하고, 유예가 끝나면 마지막 목격 위치를 기준으로 수색을 시작한다. 시야를 잃은 이후에는 플레이어의 현재 좌표를 계속 갱신하지 않는다.
...
| `Anger` 자연 감소 | `PATROL` 5초 유지 후 초당 2 | 다음 조우까지 공격성이 과도하게 유지되는지 확인 |
