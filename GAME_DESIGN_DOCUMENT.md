...
| `INVESTIGATE` | 소음 또는 힌트 위치 조사 | 시야 확보 → `CHASE`, 증거 없음 → `SEARCH`, 안전한 이탈 요청 → `RETREAT` |
...
시각 증거 수색은 마지막 목격 위치에 먼저 도착한 뒤 마지막 이동 방향으로 예측 지점을 생성한다. 초기 예측 거리는 3m이며, 이후 수색 지점의 약 65%는 이동 방향 기준 120도 부채꼴 안에서 생성한다. NavMesh 위에서 도달 가능한 지점을 만들 수 없으면 기존 무작위 수색 지점으로 대체한다. 이동 방향을 알 수 없는 청각 증거와 Director 힌트에는 방향 보정을 적용하지 않는다.
...
Director 힌트는 수신할 때와 Hint 조사 완료 후 지역 수색을 시작하기 직전에 유효 시간을 확인한다. 이동 중 만료되더라도 조사 위치까지의 이동은 즉시 취소하지 않지만, 만료된 Hint만을 근거로 추가 지역 수색을 시작하지 않고 기본 순찰로 복귀한다.
