...
`GlobalStress`는 최근 압박을 나타내는 단기 수치로만 사용한다. 발전기 복구에 따른 장기적인 난이도 상승은 Chase AI의 공격성을 나타내는 `Anger` 하한선을 높이는 방식으로 처리한다. 두 수치는 서로 다른 역할을 가지며, `Anger`가 높아져도 `GlobalStress`가 감소하면 휴식 구간은 발생할 수 있어야 한다.
...
- 근거 없이 은신처를 확인하지 않는다.
- `GlobalStress`는 Director AI의 활동과 휴식 주기를, `Anger`는 Chase AI의 공격성을 담당한다.
...
- 초기 MVP에서는 이동 속도와 수색 지속 시간 등 한두 항목에만 영향을 주며, 시야·증거 유지·수색 횟수를 동시에 강화하지 않는다.
...
- Director AI는 `GlobalStress`로 활동 주기를 결정하며 `Anger`를 출현·이탈 조건으로 직접 사용하지 않는다.
...
| `Anger` 영향 항목 | 이동 속도와 수색 지속 시간 우선 검토 | 여러 능력을 동시에 강화하지 않고 체감이 명확한지 확인 |
