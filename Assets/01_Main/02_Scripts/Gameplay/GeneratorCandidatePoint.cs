...
namespace HideSeek.Gameplay
{
    ...
    ///
    /// 후보 지점마다 발전기를 미리 배치해 비활성 상태로 두고, <see cref="GeneratorDirector"/>가
    ...
    /// 제거된 인스턴스는 <see cref="GeneratorDirector"/>가 켜고 끄지 않을 뿐, Config·QTE·위치 표시·진행도 집계는 그대로 받는다.
    ///
...
#if UNITY_EDITOR
        // 컴포넌트를 붙이는 순간 채워 둔다. 아직 Zone을 배치하지 않았을 수도 있어 실패해도 알리지 않는다.
        private void Reset()
        {
            TryResolveZoneFromPosition();
        }

        [ContextMenu("Zone/Resolve From Position")]
        private void ResolveZoneFromPosition()
        {
            if (TryResolveZoneFromPosition() == false)
            {
                Debug.LogWarning($"[{nameof(GeneratorCandidatePoint)}] '{name}'을(를) 포함하는 Zone을 찾지 못했습니다." , this);
            }
        }

        private bool TryResolveZoneFromPosition()
        {
