...
namespace HideSeek.Integration
{
    ...
    /// <summary>
    /// Chase AI들의 상태를 모아 BGM 단계를 정한다. GDD 14.1
    ///
    ...
    /// TODO: ENDING은 AI가 아니라 게임 진행도가 결정하므로 이 축에 넣지 않았다.
    ///       탈출 장치가 구현되면 별도 오버라이드로 붙인다.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class AIBgmConnector : MonoBehaviour
    {
        [Header("참조")]
        ...
        private void Start()
        {
            if (_bgmPlayer == null)
            {
                Debug.LogError($"[{nameof(AIBgmConnector)}] BgmPlayer 참조가 비어 있습니다.", this);
                enabled = false;
                return;
            }

            if (_arr_chaseAI == null || _arr_chaseAI.Length == 0)
            {
        ...
        private void ChangeState(BGM_STATE state)
        {
            ...
            Debug.Log($"[{nameof(AIBgmConnector)}] BGM 상태 전환: {state}", this);
        }
