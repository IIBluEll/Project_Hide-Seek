...
namespace HideSeek.Cutscene
{
    ...
    /// 유지하는 동안에만 켜져 있고, <see cref="ACutscenePlayer.Stop"/>이 호출되는 순간 다시 꺼진다.
    /// 결과 화면의 수명을 컷신 재생 상태와 분리하기 위해 활성화를 이쪽으로 옮겼다.
    ///
    /// 현민님의 결과 화면 흐름이 들어오면 이 컴포넌트 대신
    /// <see cref="ACutscenePlayer.Finished"/>를 직접 구독하도록 교체한다.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class CutsceneFinishedActivator : MonoBehaviour
    {
        [SerializeField] private ACutscenePlayer _cutscenePlayer;
