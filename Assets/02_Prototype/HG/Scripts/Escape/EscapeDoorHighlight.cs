...
namespace HideSeek.Gameplay
{
    ...
    ///
    /// 실루엣을 만드는 일은 <see cref="HighlightSilhouette"/>가 한다. 이 컴포넌트는 언제 켤지만 정한다.
    ...
    public sealed class EscapeDoorHighlight : MonoBehaviour
    {
        ...
        [SerializeField] private HighlightSilhouette _doorHighlight;
        ...
        private void Reset()
        {
            ...
            _doorHighlight = GetComponentInChildren<HighlightSilhouette>(true);
        }
#endif
        ...
        private void Awake()
        {
            ...
            if (_doorHighlight == null)
            {
                Debug.LogError($"[{nameof(EscapeDoorHighlight)}] 표시할 {nameof(HighlightSilhouette)}가 없습니다. 문 모델보다 상위에 붙여야 합니다." , this);
                enabled = false;
