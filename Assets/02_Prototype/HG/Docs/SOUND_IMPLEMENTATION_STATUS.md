...
| 막고 있는 것 | 영향받는 작업 |
| --- | --- |
| 씬 변경 커밋 보류 (B-3) | 전환 조건별 청취 검증 4건 |
...
**검증 방침:** 사용할 씬은 이미 결정되어 있다. 다만 여러 담당자가 같은 씬을 동시에 작업 중이라 **충돌을 피하기 위해 씬 변경을 커밋하지 않는다.** 사운드는 `AI_Prototype`에서 Play Mode와 웹 빌드로 검증만 하고, 커밋 산출물은 프리팹과 코드로만 남긴다.

즉 막혀 있는 것은 씬 선택이 아니라 **씬에 사운드를 올릴 타이밍**이다. 다른 담당자의 씬 작업이 일단락되면 프리팹을 배치하기만 하면 된다.
